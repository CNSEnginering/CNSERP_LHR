...
{
    [AbpMvcAuthorize]
    public class ControlDetailController : ERPControllerBase
    {
        ...
        public ControlDetailController(IBinaryObjectManager binaryObjectManager,
        ...
        [HttpPost]
        [AbpMvcAuthorize(AppPermissions.SetupForms_ControlDetails_Create)]
        public async Task<JsonResult> ImportFromExcel()
        {
            try
            {
                ...
                //await BackgroundJobManager.EnqueueAsync<ImportControlDetailToExcelJob, ImportControlDetailFromExcelJobArgs>(new ImportControlDetailFromExcelJobArgs
                //{
                //    TenantId = tenantId,
                ...
                var args = new ImportControlDetailFromExcelJobArgs
                {
                    TenantId = tenantId,
                    BinaryObjectId = fileObject.Id,
                    User = AbpSession.ToUserIdentifier()
                ...
                var controlDetail = _ControlDetailListExcelDataReader.GetControlDetailFromExcel(fileBytes);
                ...
                {
                    await CreateControlDetail(args, controlDetail);
                }
        ...
        private async Task CreateControlDetail(ImportControlDetailFromExcelJobArgs args, List<ImportControlDetailDto> ControlDetails)
        {
            ...
            foreach (var item in ControlDetails)
            {
                if (item.CanBeImported())
                {
                    try
                    {
                        await CreateControlDetailAsync(item);
                    }
        ...
        private async Task CreateControlDetailAsync(ImportControlDetailDto input)
        {
            ...
            if (input.Seg1ID.Length > 2)
            {
                ErrorMessage("Seg1 has invalid length");
                valid = false;
            }
