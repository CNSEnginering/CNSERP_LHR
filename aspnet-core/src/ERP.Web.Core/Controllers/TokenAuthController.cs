...
using ERP.Authentication.TwoFactor.Google;
...
using ERP.Web.Models.TokenAuth;
using ERP.Authorization.Impersonation;
using ERP.Identity;
using ERP.Net.Sms;
using ERP.Notifications;
using ERP.Web.Authentication.External;
...
namespace ERP.Web.Controllers
{
    ...
    public class TokenAuthController : ERPControllerBase
    {
        ...
        /// <summary>
        /// email sending to admin when user is logged in.
        /// this is additional working added in  Authenticate Methode by waleed .
        /// </summary>
        ...
        public async Task<AuthenticateResultModel> Authenticate([FromBody] AuthenticateModel model)
        {
            ...
            if (AbpSession.TenantId.HasValue)
            {
                if (await FeatureChecker.IsEnabledAsync((int)AbpSession.TenantId, "App.EmailAlert"))
                {
                    ...
                    EmailSettingsEditDto emailSettings = new EmailSettingsEditDto();
                    ...
                    if (loginResult.User.UserName != "admin")
                    {
                        ...
                        {
                            var MessageBody = _csAlertRepository.GetAll().Where(x => x.AlertId == 1).Count() > 0 ?
                                _csAlertRepository.GetAll().Where(x => x.AlertId == 1).FirstOrDefault().AlertBody : "";
                            ...
                            {
                                mailMessage.Body = loginResult.User.UserName + " logged in successfully";

                            }

                            var MessageSubject = _csAlertRepository.GetAll().Where(x => x.AlertId == 1).Count() > 0 ?
                                _csAlertRepository.GetAll().Where(x => x.AlertId == 1).FirstOrDefault().AlertSubject : "";
                            if (!string.IsNullOrWhiteSpace(MessageBody))
                            {
                            ...
                            else
                            {
                                ...
                                mailMessage.Subject = "User Loggin Notification";
                            }

                            await _emailSender.SendAsync(admi.Result.EmailAddress, "User Loggin Notification", loginResult.User.UserName + " logged in successfully");
            ...
            //input.UserHost = "";
            //input.LogInUser = loginResult.User.UserName;
            //input.SentDate = DateTime.Now;
            //input.CreatedBy = loginResult.User.UserName;
            //input.CreateDate = DateTime.Now;
            //input.AudtUser = loginResult.User.UserName;
